...
```text
用户确认开始巡检
-> MailClient 先读取最新 20 封邮件头（BODY.PEEK[HEADER]，不改变已读）
-> 过滤已读、本地已分类和用户规则已处理邮件
-> 对剩余候选在同一只读 IMAP 会话中按 UID 批量读取完整正文
-> 正文读取失败、缺失、超限或截断时进入人工查看，不把空正文发送给模型
-> 本地完整邮件隐私检查通过后再交给 MailAgent
...
- 邮件列表返回的 header-only / `content_truncated` 对象不能进入正文 Agent；巡检和批量分类必须先按 UID 读取完整正文。
- 摘要、翻译和草稿等按需 AI 操作也复用受限正文加载器；超限、缺失或无法读取的正文只允许人工查看。
- 生成摘要前会扫描完整邮件的本地隐私规则；普通标题但正文命中敏感/隐私规则时仍需二次确认。
- 翻译真实邮件前必须确认。
- 生成真实邮件草稿前必须确认。
- 移动到垃圾箱前必须确认。
...
这些命令覆盖真实 header-only 契约、批量正文部分失败、截断邮件 Agent 门禁、正文隐私确认、SQLite 状态迁移、原子发送 claim、CLI/Web 共享发送路径和前端交互。自动化测试不会登录真实 QQ 邮箱，不会调用真实 DeepSeek，也不会执行真实 SMTP 投递；真实服务的账号配置、网络差异和模型输出质量仍需用户明确操作后单独验证。
